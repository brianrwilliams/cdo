...
The condition that $\omega(\xi_a) = a$ simply says that $\omega$ restricts to a connection $1$-form for the principal bundle $P \to X$. 
...
Indeed, a connection $\omega \in \Omega^1(P,{\rm Lie}(K))$ determines a splitting
\ben
TP = T_\pi P \oplus H_\omega
\een
where $H_\omega \subset TP$ is defined as $\ker \omega$. That is, $H_\omega|_{p}$ is the subspace of $T_pP$ consisting of all vectors $X_p$ such that $\omega(X_p) = 0$ so that $H_{\omega}|_p \cong T_{\pi(p)}X$. Moreover, $T_\pi P$ is canonically isomorphic to the trivial bundle $\ul{{\rm Lie}(K)}$ over $P$. 
...
\een
over $X$. 
One can view it as a pairing between the category of principal $K$-bundles and the category of finite-dimensional $K$-modules, i.e., a functor
\[
...
We can explicitly describe the action by a closed two-form $f(t_1,\ldots,t_n) \d t_{j_1} \d t_{j_2}$ by
\ben
...
Following \cite{GMS}, we give an explicit formula via a pair of linear maps $(\alpha_2, \alpha_3)$, where 
\ben
\begin{array}{cccc}
\alpha_2 : & \Aut_n \times \Aut_n & \to & \hOmega^{2}_n \\ 
& (f_1,f_2) & \mapsto&\tr\left(g_1^{-1} \d_{dR} g_1 (\d_{dR}g_2 g_2^{-1})\right)
\end{array}
\een
and
\ben
...
& f & \mapsto & \frac{1}{3} \tr\left( (g^{-1}\d_{dR} g)^3\right) .
\end{array}
\een
This cochain is of degree 2 and has no terms of type $\Omega^k$ for $k \geq 4$. Here $g_1, g_2, g$ are the Jacobians of $f_1,f_2,f$, respectively. One
...
\circ f_1$ is given by $Jac(f_2 \circ f_1) = f_1^* (g_2) g_1$ as matrix-valued formal power series. 
Thus, for instance, we have
\ben
\d_{dR} \left(Jac(f_2 \circ f_1)\right) = f_1^*(\d_{dR} g_2) g_1 + f_1^* g_2
\d_{dR} g_1 . 
\een 
Let $g = Jac(f_2 \circ f_1)$ so that $\alpha_3(f_2 \circ f_1) =
\frac{1}{3} \Tr\left((g^{-1} \d_{dR} g)^3\right)$. Plugging in the formula for the
Jacobian we compute
\bestar
\frac{1}{3} \left(g^{-1} \d_{dR} g\right)^3 & = & \frac{1}{3} \left(g_1^{-1} f_1^*(g_2^{-1} \d_{dR}
g_2) g_1\right)^3 + \frac{1}{3} \left(g_1^{-1} \d_{dR} g_1\right)^3 +
\{{\rm cross \; terms}\} .
\eestar
...
the formal Poincar\'e lemma assures the existence of a linear map
...
\ben
{\rm Hoch}_*\left(\clie^*(\fg)\right) \simeq \clie^*\left(\fg ; \oplus_{k \geq 0} \Sym^k (\fg^\vee)[k] \right) . 
\een
...
\ben
\chi^{\fg} (W) := {\rm Td}^\fg(\fg[1]) \cdot \ch^\fg(W) \in \clie^*\left(\fg ; \oplus_{k \geq 0} \Sym^k (\fg^\vee)[k] \right) .
\een
...
\begin{cor}\label{cor formal char cdo} The $(\TVect, \GL_n)$-equivariant graded character of the vertex algebra $\hCDO_n$ is the image of an element 
\ben
...
\ben
\Gr \; \hCDO_n \cong
\underset{{0<k}}{\widehat{\bigotimes}}\,\cSym_{\hO_n}(\hOmega^1_n)\,
\widehat{\otimes}\, \underset{{0\leq
    l}}{\widehat{\bigotimes}}\,\cSym_{\hO_n}(\hT_n) .
\een
...
\ben
\CS (A) := {\rm Tr}\left(A \wedge \dbar A + \frac{1}{3} A\wedge A \wedge
  A\right)
\een
