...
\begin{rmk} 
The Gelfand-Kazhdan approach to formal geometry can also be applied to more interesting geometries. For example, symplectic, Poisson, or even Riemannian geometry can be encapsulated by the formalism.
\end{rmk}
...
In particular, if $\fg = \Lie(K)$, then condition (1) encodes the usual notion of a connection on a principal $K$-bundle, while condition (2) says that the connection is {\em flat}, i.e., we have a principal bundle for the discrete group~$K^\delta$ underlying~$K$.
...
\begin{ex}
Let $G$ be a Lie group and let $K \subset G$ be a closed Lie subgroup. 
Then the $K$-principal bundle $G \to G/K$ has the natural structure of a $(\fg,K)$-principal bundle where $\fg = {\rm Lie}(G)$. 
When $G$ is compact, there is a quasi-isomorphism $\bdesc(G \to G/K, V) \simeq \clie^*(\fg, K ; V)$ for any $G$-representation $V$. 
This quasi-isomorphism is a relative version of the standard fact that the differential forms on $G$ are quasi-isomorphic to the absolute Chevalley-Eilenberg complex~$\clie^*(\fg)$. 
\end{ex}
...
\]
where $\sO_{X,x}^\wedge$ denotes the completion $\lim_{\leftarrow} \sO_{X,x}/\fm_x^k$ of the germ at $x$ of holomorphic functions with respect to powers of the ideal $\fm_x$ of functions vanishing at~$x$.  
...
These isomorphisms glue together to give the following. 
...
\begin{rmk}
Following Section 4 of \cite{kapranov1999}, 
we can realize $\Exp (X)$ as the inverse limit of finite dimensional manifolds $\Exp_k (X) = X^{coor}_k / \GL_n$.
...
The issue is that this product is not associative!
...
A slight modification of Lemma \ref{lem: offdiag}, which applies to the $q$-graded situation, implies the following. 
...
\begin{rmk} 
The $2$-form $\rho$ is only unique up to a holomorphic closed
...
\begin{rmk}
\label{rmk: closed2}
It is here that we see the explicit appearance of closed $2$-forms, or really its natural resolution. 
\end{rmk}
...
\begin{rmk} 
Note that the choice of $\Hat{\rho}_{f, \varphi}$ is only unique up to a closed holomorphic $2$-form on the formal disk. That is, for each $\eta \in \hOmega^2_{n,cl}$ we get a different action of $\Tilde{\Aut}_n$ defined by
\ben
...
$\sigma_{\Omega^2} \circ \sigma$. This composite defines the connection one-form
\ben
...
\begin{rmk} 
The descent functor $\Tilde{\desc}_{\GK}$ depends on the
