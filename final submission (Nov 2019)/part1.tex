...
\begin{ex}
Suppose $G$ is a Lie group and $K \subset G$ is a subgroup. 
Then, the $K$-principal bundle $G \to G/K$ has the natural structure of a $(\fg,K)$-principal bundle, $\fg = {\rm Lie}(G)$. 
When $G$ is compact, there is an isomorphism $\bdesc(G \to G/K, V) \cong \clie^*(g, K ; V)$ for any $G$-representation $V$. 
This is a relative version of the standard fact that the differential forms on $G$ are isomorphic to the absolute Chevalley-Eilenberg complex $\clie^*(\fg)$. 
\end{ex}
